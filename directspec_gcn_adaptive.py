...
parser.add_argument('--tau_0', type=float, default=2.5, help='tau_0')
parser.add_argument('--tau_1', type=float, default=3.0, help='tau_1')
parser.add_argument('--shrink_norm', type=float, default=1.0, help='shrinking the norm of embeddings')
...
class DirectSpec(nn.Module):
	...
	def __init__(self, user_size, item_size, dataset, latent_size=64, alpha=1.0, tau_0=2.5,tau_1=3.0,shrink_norm=0.0, batch_size=512,learning_rate=10):
		...
		self.L=torch.Tensor(np.load("./"+self.dataset+r'/L_3_80.npy')).cuda()
		self.user_con_weight=((self.rate_matrix.mm(self.rate_matrix.t()))==0)*(tau_1-tau_0)+tau_0
		self.item_con_weight=((self.rate_matrix.t().mm(self.rate_matrix))==0)*(tau_1-tau_0)+tau_0
	...
	def forward(self,u,p):
		...
		final_user,final_pos=self.L[u].mm(self.embed),self.L[user+p].mm(self.embed)

		final_user=final_user /final_user.norm(2,dim=1).unsqueeze(1)
		final_pos=final_pos/final_pos.norm(2,dim=1).unsqueeze(1)
		
		
		user_con=self.user_con_weight[u][:,u]
		item_con=self.item_con_weight[p][:,p]

		user_correlation=torch.exp(final_user.mm(final_user.t())*user_con)
		item_correlation=torch.exp(final_pos.mm(final_pos.t())*item_con)

		
		final_user = final_user - self.alpha* (user_correlation/user_correlation.sum(1).unsqueeze(1)).mm(final_user)

		final_pos = final_pos - self.alpha* (item_correlation/item_correlation.sum(1).unsqueeze(1)).mm(final_pos)
...
model=DirectSpec(user,item,dataset,embedding_size,alpha,tau_0,tau_1,shrink_norm, batch_size,lr,reg)
